...
import { gridworldPythonCode, type PythonCodeEntry } from '../../content/gridworldPythonCode'
...
export function GridWorldPythonTab({ algorithmType, onAlgorithmChange }: Props) {
  const entry: PythonCodeEntry = gridworldPythonCode[algorithmType]
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `gridworld_${entry.id.replace(/-/g, '_')}.py`
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {(Object.keys(gridworldPythonCode) as AlgorithmType[]).map((type) => (
            <button
              key={type}
